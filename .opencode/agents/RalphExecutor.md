...
Work on EXACTLY what @RalphCoordinator assigned:
...
After committing, provide a concise summary to @RalphCoordinator:
...
Keep summary concise - @RalphCoordinator only needs completion status and key details.
...
- Return concise summary to @RalphCoordinator
...
5. Only then return summary to @RalphCoordinator
...
**NEVER output** `<promise>COMPLETE</promise>` - only @RalphCoordinator does that.

Your job: execute task, update progress, return summary. @RalphCoordinator handles the loop.
